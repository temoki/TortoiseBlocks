...
| M | 内容 | 完了条件 |
|---|---|---|
| **M0** | プロジェクト雛形：xcodeproj + Kit パッケージ + TortoiseGraphics2 依存。ハードコードしたコマンド列が `TortoiseCanvas(_:player:)` で動く walking skeleton | iOS/macOS 両方でビルド・実行でき、pause 中 `step()` の再描画を実機確認済み（§15） |
...
| **M3** | 実行体験：実行 / 一時停止 / ステップ / シーク / 速度スライダー（ライブ変更）＋実行中ブロックの常時ハイライト（`TortoisePlayer` ベース） | くりかえし＋乱数の作品が動く |
