...
裏を返すと、これは自分の名前で書く公開レビューなので、**本当に使ったものだけ**を
挙げる。以下はすべてこのリポジトリに裏付けがある。

| プロダクト | 裏付け |
|---|---|
| **Swift Playgrounds** | 2016年、Tortoise Graphics を Playgrounds Book として配った（[why.html](../../site/why.html)） |
| **Swift** | アプリもエンジンも、コードペインが見せるものも Swift |
| **Xcode** | 1プロジェクトで3プラットフォーム、Xcode Cloud がタグから配信 |
| **Blender** | Vision Pro の3Dのカメは `Tools/` の Blender スクリプトが出力している |
| **Figma** | Mac のキャプチャは Figma で組んでいる（`Tools/screenshots.rb` のコメント） |
| **GitHub** | リポジトリ、CI、リリース、そして Pages のサイトそのもの |
| **Claude Code** | `CLAUDE.md` 一式。ただし下の注意を読んでから決める |

**Claude Code だけは判断が要る。** 事実として使っているし、Product Hunt では
被リンクの見られる量が一番多い相手でもある。一方でこのアプリは「子ども向け」で
「作りの丁寧さ」を売りにしているので、AI で作ったと読まれることを嫌う層もいる。
外すなら6つ、入れるなら7つ。

### レビュー文

そのまま貼れるもの。どれも具体的な事実を1つ含むようにしてある
（褒め言葉だけのレビューは読まれないし、残らない）。

**Swift Playgrounds**

```
Tortoise Blocks started here. In 2016 I wrote a turtle graphics engine in
Swift and shipped it as a Playgrounds Book, so a child could try it on an iPad
with nothing to install. Two schools wrote to say they were teaching with it.
This app is what that grew into.
```

**Swift**

```
The app, the engine under it, and the code it shows children are all Swift.
The whole point of the app is that a child's blocks and a programmer's Swift
are one program in two notations — which only works because the language reads
well enough to put in front of a twelve-year-old.
```

**Xcode**

```
One project, three platforms — iPad, Mac and Apple Vision Pro — and Xcode
Cloud archives and ships all three from a git tag. A solo developer gets a
real release pipeline out of it without building one.
```

**Blender**

```
The tortoise that stands on the paper on Apple Vision Pro is a Blender model,
generated by a script that lives in the repository — so the mascot can be
...
```
Every Mac screenshot in the App Store listing is composed in Figma: the window
on its plate, at the exact pixel size Apple asks for. The captures come out of
the simulator; Figma is what turns them into a listing.
```

**GitHub**

```
The repository, the CI that checks every pull request on three platforms, the
releases, and the website itself on Pages. The app is MIT there, which is also
the honest answer when a parent asks what a kids' app does with their child's
data: read it.
```
...
```
Built alongside Claude Code, which knows this repository through a CLAUDE.md
that records the decisions rather than the code — the frozen document format,
the recursion limit that was measured rather than chosen, the traps that fail
silently. That file is why the help is still useful months later.
```
...
- [ ] Shoutouts に Claude Code を入れるか決める
