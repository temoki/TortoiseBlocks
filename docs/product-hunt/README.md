...
裏を返すと、これは自分の名前で書く公開レビューなので、本当に使ったものだけを挙げる。

フォームで実在を確認して選んだのは次の5つ:

| プロダクト | 裏付け |
|---|---|
| SwiftUI Apps | 3プラットフォームが同じ1つのレイアウト |
| Swift Playgrounds | 2016年、Tortoise Graphics を Playgrounds Book として配った |
| Blender | Vision Pro の3Dのカメは `Tools/` の Blender スクリプトが出力している |
| Figma | Mac のキャプチャは Figma で組んでいる（`Tools/screenshots.rb` のコメント） |
| GitHub | リポジトリ、CI、リリース、Pages のサイトそのもの |

Claude Code は入れないことにした。Swift と Xcode は SwiftUI Apps と
Swift Playgrounds に含めた形になっている。

**SwiftUI Apps はフレームワークそのものではなく「SwiftUI で作られたアプリの
コレクション」のリストである**点だけ注意。カタログにフレームワーク単体の項目が
ないので代わりに使っているが、レビュー文は SwiftUI を選んだ理由として書いてある。

### レビュー文

それぞれ追加で「なぜ他ではなくこれを選んだのか」と聞かれる。以下はその答えとして
書いたもので、そのまま貼れる。どれも具体的な事実を1つ含むようにしてある
（褒め言葉だけのレビューは読まれないし、残らない）。

**What made you choose SwiftUI Apps over the alternatives?**

```
Because one layout had to serve three very different machines. Tortoise Blocks
is the same three-pane app on iPad, Mac and Apple Vision Pro, and almost none
of that layout is conditional — visionOS runs the iPad app rather than a port
of it. With UIKit and AppKit that is two codebases and a third guess; with a
cross-platform toolkit it is three places that feel like none of them. SwiftUI
also hands a document app the parts nobody wants to write: Files and Finder
integration, autosave, system undo. What platform difference is left comes to
about a dozen #if lines, and every one of them is a missing API rather than a
layout decision.
```

**What made you choose Swift Playgrounds over Xcode?**

```
They were never competing for the same job. Xcode is where this app is built;
Swift Playgrounds is how the thing it grew out of reached a child in the first
place. In 2016 I wrote a turtle graphics engine in Swift and shipped it as a
Playgrounds Book, which meant a kid could run real Swift on an iPad they
already had — no Mac, no developer account, nothing to install. Xcode could not
have put it in front of that child. Two schools wrote to say they were teaching
with it, and the order one of their courses ran in is the design of this app.
```

**What made you choose Blender over the alternatives?**
...
**What made you choose Figma over the alternatives?**
